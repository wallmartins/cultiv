...
import { Effect as Effect127 } from "effect";
...
function createAuditRepository(stateRef) {
  return {
    putIfAbsent(record) {
      ...
      if (existing) {
        return Effect59.succeed(cloneRecord(existing));
      }
      stateRef.current = {
        ...stateRef.current,
        auditRecords: {
      ...
      return Effect59.succeed(cloneRecord(record));
    },
    getByLogicalKey(logicalKey) {
      ...
      return Effect59.succeed(record ? cloneRecord(record) : void 0);
    },
    list() {
      return Effect59.succeed(
...
function createContentTypeRepository(stateRef) {
  return {
    put(record, version = 1, updatedAt = (/* @__PURE__ */ new Date()).toISOString()) {
      ...
      return Effect60.succeed(cloneRecord(next));
    },
    get(id) {
    ...
    list() {
      return Effect60.succeed(Object.values(stateRef.current.contentTypes).map(cloneRecord));
    }
  };
}
  ...
  return {
    create(job, options = {}) {
      if (stateRef.current.jobs[job.id]) {
        return Effect61.fail(new DatabaseJobAlreadyExistsError({ jobId: job.id }));
      }
      ...
      return Effect61.succeed(cloneRecord(record));
    },
    save(record) {
    ...
    list() {
      return Effect61.succeed(Object.values(stateRef.current.jobs).map(cloneRecord));
    },
    listByUser(_userId, limit, offset) {
      ...
      return Effect61.succeed(records.slice(offset, offset + limit));
    },
    countByUser(_userId) {
      return Effect61.succeed(Object.keys(stateRef.current.jobs).length);
    },
    remove(id) {
      if (!stateRef.current.jobs[id]) {
        return Effect61.succeed(false);
      }
...
function requireJob(state, id) {
  ...
  if (!record) {
    return Effect61.fail(new DatabaseJobNotFoundError({ jobId: id }));
  }
  return Effect61.succeed(record);
}
...
function createMemoryRepository(stateRef) {
  return {
    put(record, version = 1) {
      ...
      return Effect62.succeed(cloneRecord(next));
    },
    get(userId, key) {
...
function createPipelineRepository(stateRef) {
  return {
    ...
    list() {
      return Effect63.succeed(Object.values(stateRef.current.pipelines).map(cloneRecord));
    }
  };
}
  ...
  return {
    create(record, version = 1) {
      if (stateRef.current.voiceExampleBatches[record.id]) {
        return Effect64.fail(new DatabaseVoiceBatchAlreadyExistsError({ batchId: record.id }));
      }
    ...
    save(record) {
      if (!stateRef.current.voiceExampleBatches[record.id]) {
        return Effect64.fail(new DatabaseVoiceBatchNotFoundError({ batchId: record.id }));
      }
      const next = {
        ...record,
  ...
  return {
    create(record, version = 1) {
      if (stateRef.current.voiceExamples[record.id]) {
        return Effect65.fail(new DatabaseVoiceExampleAlreadyExistsError({ exampleId: record.id }));
      }
    ...
    save(record) {
      if (!stateRef.current.voiceExamples[record.id]) {
        return Effect65.fail(new DatabaseVoiceExampleNotFoundError({ exampleId: record.id }));
      }
      const next = {
        ...record,
        version: record.version + 1
    ...
    removeByUser(userId) {
      ...
      return Effect65.succeed(removedCount);
    }
  };
}
...
function createVoiceProfileSnapshotRepository(stateRef) {
  return {
    ...
    removeByUser(userId) {
      ...
      return Effect68.succeed(removedCount);
    }
  };
}
...
function createClient(state) {
  ...
  const client = {
    ...
    transaction: (operation) => Effect70.gen(function* () {
...
import { Context as Context11, Effect as Effect72, Layer as Layer10 } from "effect";
...
import { Effect as Effect73 } from "effect";
...
import { Cause as Cause2, Effect as Effect85, Exit, Option as Option2 } from "effect";
  ...
  return {
    create(job, options = {}) {
      return Effect74.gen(function* () {
        const existing = yield* Effect74.tryPromise({
          try: () => db.selectFrom("jobs").where("id", "=", job.id).selectAll().executeTakeFirst(),
          catch: () => void 0
        }).pipe(Effect74.catchAll(() => Effect74.succeed(void 0)));
        if (existing) {
          return yield* Effect74.fail(new DatabaseJobAlreadyExistsError({ jobId: job.id }));
        }
    ...
    save(record) {
      return Effect74.gen(function* () {
        const current = yield* Effect74.tryPromise({
          try: () => db.selectFrom("jobs").where("id", "=", record.id).selectAll().executeTakeFirst(),
          catch: () => void 0
        }).pipe(Effect74.catchAll(() => Effect74.succeed(void 0)));
        if (!current) {
          return yield* Effect74.fail(new DatabaseJobNotFoundError({ jobId: record.id }));
        }
    ...
    findById(id) {
      return Effect74.gen(function* () {
        const row = yield* Effect74.tryPromise({
    ...
    list() {
      return Effect74.gen(function* () {
        const rows = yield* Effect74.tryPromise({
          try: () => db.selectFrom("jobs").selectAll().execute(),
          catch: () => []
        }).pipe(Effect74.catchAll(() => Effect74.succeed([])));
    ...
    countByUser(userId) {
      return Effect74.gen(function* () {
        ...
        }).pipe(Effect74.catchAll(() => Effect74.succeed({ count: 0 })));
        return Number(row?.count ?? 0);
      });
    },
    remove(id) {
      return Effect74.gen(function* () {
        const result = yield* Effect74.tryPromise({
          try: () => db.deleteFrom("jobs").where("id", "=", id).executeTakeFirst(),
          catch: () => ({ numDeletedRows: 0n })
        }).pipe(Effect74.catchAll(() => Effect74.succeed({ numDeletedRows: 0n })));
    ...
    appendHistory(id, entry) {
      return Effect74.gen(function* () {
        ...
        if (!current) {
          return yield* Effect74.fail(new DatabaseJobNotFoundError({ jobId: id }));
        }
  ...
  return {
    create(record, version = 1) {
      return Effect78.gen(function* () {
        const existing = yield* Effect78.tryPromise({
        ...
        if (existing) {
          return yield* Effect78.fail(new DatabaseVoiceExampleAlreadyExistsError({ exampleId: record.id }));
        }
    ...
    save(record) {
      return Effect78.gen(function* () {
        ...
        if (!existing) {
          return yield* Effect78.fail(new DatabaseVoiceExampleNotFoundError({ exampleId: record.id }));
        }
    ...
    remove(id) {
      return Effect78.gen(function* () {
        const result = yield* Effect78.tryPromise({
          try: () => db.deleteFrom("voice_examples").where("id", "=", id).executeTakeFirst(),
          catch: () => ({ numDeletedRows: 0n })
        }).pipe(Effect78.catchAll(() => Effect78.succeed({ numDeletedRows: 0n })));
        return result.numDeletedRows > 0n;
      });
    },
    removeByUser(userId) {
  ...
  return {
    create(record, version = 1) {
      return Effect82.gen(function* () {
        const existing = yield* Effect82.tryPromise({
        ...
        if (existing) {
          return yield* Effect82.fail(new DatabaseVoiceBatchAlreadyExistsError({ batchId: record.id }));
        }
        const next = { ...record, version };
    ...
    save(record) {
      return Effect82.gen(function* () {
        ...
        if (!existing) {
          return yield* Effect82.fail(new DatabaseVoiceBatchNotFoundError({ batchId: record.id }));
        }
  ...
  return {
    putIfAbsent(record) {
      return Effect84.gen(function* () {
        const existing = yield* Effect84.tryPromise({
        ...
        const stored = yield* Effect84.tryPromise({
          try: () => db.selectFrom("audit_records").where("logical_key", "=", record.logicalKey).selectAll().executeTakeFirst(),
          catch: () => void 0
        }).pipe(Effect84.catchAll(() => Effect84.succeed(void 0)));
    ...
    list() {
      return Effect84.gen(function* () {
        const rows = yield* Effect84.tryPromise({
          try: () => db.selectFrom("audit_records").selectAll().orderBy("occurred_at", "asc").execute(),
          catch: () => []
        }).pipe(Effect84.catchAll(() => Effect84.succeed([])));
...
import { Effect as Effect86 } from "effect";
...
function validateSchema(db, expectedTables) {
  return Effect86.gen(function* () {
    const tables = yield* Effect86.tryPromise({
      try: async () => {
...
function hasPostgresBillingTables(db) {
  ...
  }).pipe(Effect88.orElseSucceed(() => false));
}
  ...
  return Effect89.gen(function* () {
    ...
    if (relationalEnabled) {
      const relational = yield* loadPostgresBillingRepository(db).pipe(
        Effect89.catchAll(() => Effect89.succeed(createBillingRepository()))
      );
...
function saveBillingRepository(db, repository, updatedAt) {
  return Effect89.tryPromise({
    try: () => runBillingRepositoryPersistSerialized(
      () => Effect89.runPromise(saveBillingRepositoryUnqueued(db, repository, updatedAt))
    ),
    catch: (error) => error instanceof Error ? error : new Error(String(error))
  });
}
...
function createPersistingBillingService(postgres, repository, now) {
  ...
  const schedulePersist = () => {
    scheduleBillingRepositoryPersist(() => Effect90.runPromise(persistUnqueued()));
  };
  return {
    registerPlan(plan) {
      return billing.registerPlan(plan).pipe(Effect90.tap(() => persistSilently()));
    },
    upsertSubscription(subscription) {
    ...
    consumeCredits(userId, planId, amount, kind) {
      return billing.consumeCredits(userId, planId, amount, kind).pipe(Effect90.tap(() => persistSilently()));
    },
    quoteDebitForMode: billing.quoteDebitForMode.bind(billing),
    startCycle(request) {
      return billing.startCycle(request).pipe(Effect90.tap(() => persistSilently()));
    },
    reserveGenerationCredits(request) {
      return billing.reserveGenerationCredits(request).pipe(Effect90.tap(() => persistSilently()));
    },
    captureReservedCredits(request) {
      return billing.captureReservedCredits(request).pipe(Effect90.tap(() => persistSilently()));
    },
    releaseReservedCredits(request) {
      return billing.releaseReservedCredits(request).pipe(Effect90.tap(() => persistSilently()));
    },
    registerTopUpPackage(pkg) {
    ...
    purchaseTopUp(request) {
      return billing.purchaseTopUp(request).pipe(Effect90.tap(() => persistSilently()));
    },
...
import { Effect as Effect92, Schema as Schema8 } from "effect";
...
import { Effect as Effect91, ParseResult, Schema } from "effect";
...
var VoiceProfileConfidenceSchema = Schema2.Literal("low", "medium", "high");
var VoiceAdaptationModeSchema = Schema2.Literal("conservative", "standard");
var VoiceExampleStateSchema = Schema2.Literal("active", "excluded");
var AttentionLevelSchema = Schema2.Literal("low", "medium", "high");
var ReasonCodeSchema = Schema2.Literal(
...
var NextActionCodeSchema = Schema2.Literal(
  "add_more_examples",
...
var ContributionCodeSchema = Schema2.Literal(
...
var AttentionReasonCodeSchema = Schema2.Literal(
  "redundant_example",
  "too_short",
  "low_specificity",
...
var FallbackReasonCodeSchema = Schema2.Literal("rebuild_failed", "rebuild_in_progress");
var VoiceCoverageSchema = Schema2.Literal("low", "medium", "high");
var VoiceSignalSummarySchema = Schema2.Struct({
  styleMarkers: Schema2.Array(Schema2.String),
  rules: Schema2.Array(Schema2.String),
  antiPatterns: Schema2.Array(Schema2.String)
});
var VoiceProfileViewSchema = Schema2.Struct({
  userId: Schema2.String,
  snapshotId: Schema2.String,
  version: Schema2.Number,
  confidence: VoiceProfileConfidenceSchema,
  adaptationMode: VoiceAdaptationModeSchema,
  primaryLanguage: Schema2.String,
  tone: Schema2.String,
  cadence: Schema2.String,
  description: Schema2.optional(Schema2.String),
  lexicon: Schema2.Array(Schema2.String),
  constraints: Schema2.Array(Schema2.String),
  styleMarkers: Schema2.Array(Schema2.String),
  rules: Schema2.Array(Schema2.String),
  antiPatterns: Schema2.Array(Schema2.String)
});
var VoiceMaterialBaseBreakdownSchema = Schema2.Struct({
  totalExamples: Schema2.Number,
  activeExamples: Schema2.Number,
  excludedExamples: Schema2.Number,
  pinnedExamples: Schema2.Number,
  byClassification: Schema2.Record({ key: Schema2.String, value: Schema2.Number }),
  byContentType: Schema2.Record({ key: Schema2.String, value: Schema2.Number }),
  byLanguage: Schema2.Record({ key: Schema2.String, value: Schema2.Number })
});
var VoiceCoverageItemViewSchema = Schema2.Struct({
  contentType: Schema2.String,
  coverage: VoiceCoverageSchema,
  reasonCodes: Schema2.Array(ReasonCodeSchema)
});
var VoiceProfileDiagnosticsViewSchema = Schema2.Struct({
  updating: Schema2.Boolean,
  activeVersion: Schema2.Number,
  pendingVersion: Schema2.optional(Schema2.Number),
  summary: Schema2.optional(Schema2.String),
  reasonCodes: Schema2.Array(ReasonCodeSchema),
  nextActionCodes: Schema2.Array(NextActionCodeSchema),
  bestCoveredContentTypes: Schema2.Array(VoiceCoverageItemViewSchema),
  underrepresentedContentTypes: Schema2.Array(VoiceCoverageItemViewSchema),
  pendingRebuild: Schema2.Struct({
    status: Schema2.Literal("idle", "in_progress", "failed"),
    reasonCode: Schema2.optional(ReasonCodeSchema),
    nextActionCodes: Schema2.Array(NextActionCodeSchema)
  })
});
var VoiceProfileScreenViewSchema = Schema2.Struct({
  profile: VoiceProfileViewSchema,
  diagnostics: VoiceProfileDiagnosticsViewSchema,
  materialBase: VoiceMaterialBaseBreakdownSchema
});
var VoiceExampleEvaluationViewSchema = Schema2.Struct({
  systemWeight: Schema2.Number,
  attentionLevel: AttentionLevelSchema,
  attentionReasonCodes: Schema2.Array(AttentionReasonCodeSchema),
  contributionCode: ContributionCodeSchema,
  contributionPreview: Schema2.String,
  userPinned: Schema2.Boolean
});
var VoiceExampleListItemViewSchema = Schema2.Struct({
  exampleId: Schema2.String,
  version: Schema2.Number,
  state: VoiceExampleStateSchema,
  text: Schema2.String,
  previewText: Schema2.String,
  language: Schema2.String,
  channel: Schema2.optional(Schema2.String),
  format: Schema2.optional(Schema2.String),
  explicitContentType: Schema2.optional(Schema2.String),
  effectiveContentTypeHints: Schema2.Array(Schema2.String),
  classificationLabels: Schema2.Array(Schema2.String),
  pinned: Schema2.Boolean,
  pendingProfileImpact: Schema2.Boolean,
  targetProfileVersion: Schema2.optional(Schema2.Number),
  evaluation: VoiceExampleEvaluationViewSchema,
  createdAt: Schema2.String,
  updatedAt: Schema2.String
});
var VoiceExamplesPageViewSchema = Schema2.Struct({
  items: Schema2.Array(VoiceExampleListItemViewSchema),
  total: Schema2.Number,
  limit: Schema2.Number,
  offset: Schema2.Number
});
var VoiceExampleCreateInputSchema = Schema2.Struct({
  text: Schema2.String,
  language: Schema2.optional(Schema2.String),
  channel: Schema2.optional(Schema2.String),
  format: Schema2.optional(Schema2.String),
  explicitContentType: Schema2.optional(Schema2.String),
  context: Schema2.optional(Schema2.String),
  antiPatternsExplicit: Schema2.optional(Schema2.Array(Schema2.String)),
  userLabels: Schema2.optional(Schema2.Array(Schema2.String)),
  pinned: Schema2.optional(Schema2.Boolean),
  performance: Schema2.optional(
    Schema2.Struct({
      channel: Schema2.optional(Schema2.String),
      publishedAt: Schema2.optional(Schema2.String),
      selfRating: Schema2.optional(Schema2.Number),
      likes: Schema2.optional(Schema2.Number),
      comments: Schema2.optional(Schema2.Number)
    })
  )
});
var VoiceExampleUpdateInputSchema = Schema2.Struct({
  text: Schema2.optional(Schema2.String),
  language: Schema2.optional(Schema2.String),
  channel: Schema2.optional(Schema2.String),
  format: Schema2.optional(Schema2.String),
  explicitContentType: Schema2.optional(Schema2.String),
  ...
  userLabels: Schema2.optional(Schema2.Array(Schema2.String)),
  pinned: Schema2.optional(Schema2.Boolean),
  state: Schema2.optional(VoiceExampleStateSchema)
});
var VoiceExampleBatchCreateInputSchema = Schema2.Struct({
  expiresAt: Schema2.optional(Schema2.String)
});
var VoiceExampleBatchItemInputSchema = Schema2.Struct({
  clientItemId: Schema2.String,
  input: VoiceExampleCreateInputSchema
});
var VoiceExampleBatchItemsInputSchema = Schema2.Struct({
  items: Schema2.Array(VoiceExampleBatchItemInputSchema)
});
var VoiceExampleBatchItemResultViewSchema = Schema2.Struct({
  clientItemId: Schema2.String,
  accepted: Schema2.Boolean,
  exampleId: Schema2.optional(Schema2.String),
  reasonCode: Schema2.optional(ReasonCodeSchema),
  message: Schema2.optional(Schema2.String)
});
var VoiceExampleBatchViewSchema = Schema2.Struct({
  batchId: Schema2.String,
  status: Schema2.Literal("open", "committed", "expired"),
  expiresAt: Schema2.String,
  acceptedItems: Schema2.Number,
  rejectedItems: Schema2.Number,
  itemResults: Schema2.Array(VoiceExampleBatchItemResultViewSchema)
});
var VoiceExampleBatchCommitResultViewSchema = Schema2.Struct({
  batchId: Schema2.String,
  committedAt: Schema2.String,
  acceptedItems: Schema2.Number,
  rejectedItems: Schema2.Number,
  targetProfileVersion: Schema2.optional(Schema2.Number)
});
...
var VoiceTrainingConsentStatusViewSchema = Schema2.Struct({
  granted: Schema2.Boolean,
  grantedAt: Schema2.optional(Schema2.String),
  revokedAt: Schema2.optional(Schema2.String)
});
...
var ApiErrorCategorySchema = Schema3.Literal(
  "invalid_request",
  "authentication",
  "authorization",
  "not_found",
...
var ApiErrorCodeSchema = Schema3.Literal(
  "invalid_request",
...
var ApiErrorResponseSchema = Schema3.Struct({
  status: Schema3.Number,
  code: ApiErrorCodeSchema,
  category: ApiErrorCategorySchema,
  message: Schema3.String,
  retryable: Schema3.Boolean,
  details: Schema3.optional(Schema3.Record({ key: Schema3.String, value: Schema3.Unknown }))
});
var PendingVoiceProfileRebuildViewSchema = Schema3.Struct({
...
var ExecutionVoiceMetadataViewSchema = Schema3.Struct({
  voiceProfileConfidence: VoiceProfileConfidenceSchema,
  voiceAdaptationMode: VoiceAdaptationModeSchema,
  voiceProfileVersionUsed: Schema3.Number,
  pendingVoiceProfileVersion: Schema3.optional(Schema3.Number),
  voiceProfileSnapshotId: Schema3.String,
  usedFallbackVoiceProfile: Schema3.Boolean,
  fallbackReasonCode: Schema3.optional(FallbackReasonCodeSchema),
  appliedSignals: VoiceSignalSummarySchema,
  pendingProfileRebuild: PendingVoiceProfileRebuildViewSchema
});
var JobStatusResponseSchema = Schema3.Struct({
  jobId: Schema3.String,
  status: JobStatusSchema,
  contentType: Schema3.String,
  progress: Schema3.NullOr(JobProgressSchema),
  result: Schema3.NullOr(JobResultSchema),
  error: Schema3.NullOr(JobErrorSchema),
  createdAt: Schema3.String,
  completedAt: Schema3.NullOr(Schema3.String),
  voice: Schema3.optional(ExecutionVoiceMetadataViewSchema),
  userId: Schema3.optional(Schema3.String)
});
var SSEEventSchema = Schema3.Struct({
  type: Schema3.Literal("progress", "done", "error"),
  jobId: Schema3.String,
  payload: Schema3.Union(JobProgressSchema, JobResultSchema, JobErrorSchema)
});
var ExecutionSseEventSchema = Schema3.Struct({
  type: Schema3.Literal("progress", "done", "error"),
  payload: Schema3.Union(JobProgressSchema, JobResultSchema, JobErrorSchema),
  occurredAt: Schema3.String
});
var SimplifiedPipelineRequestSchema = Schema3.Struct({
  userId: Schema3.String,
  pipelineType: PipelineTypeSchema,
  briefing: Schema3.Union(Schema3.String, Schema3.Record({ key: Schema3.String, value: Schema3.Unknown })),
  importedContext: Schema3.optional(Schema3.String),
  context: Schema3.optional(Schema3.Record({ key: Schema3.String, value: Schema3.Unknown })),
  language: Schema3.optional(Schema3.String),
  qualityMode: Schema3.optional(QualityModeSchema),
  contentType: Schema3.optional(Schema3.String),
  model: Schema3.optional(Schema3.String),
  adapter: Schema3.optional(Schema3.String),
  quoteId: Schema3.optional(Schema3.String),
  previewRecommendation: Schema3.optional(PreviewRecommendationSchema),
  includeTrace: Schema3.optional(Schema3.Boolean),
  idempotencyKey: Schema3.optional(Schema3.String)
});
var ExplicitPipelineRequestSchema = Schema3.Struct({
  pipeline: PipelineDefinitionSchema,
  importedContext: Schema3.optional(Schema3.String),
  context: Schema3.optional(Schema3.Record({ key: Schema3.String, value: Schema3.Unknown })),
  inputs: Schema3.optional(Schema3.Record({ key: Schema3.String, value: Schema3.Unknown })),
  language: Schema3.optional(Schema3.String),
  qualityMode: Schema3.optional(QualityModeSchema),
  model: Schema3.optional(Schema3.String),
  adapter: Schema3.optional(Schema3.String),
...
var PipelineRequestSchema = Schema3.Union(
  SimplifiedPipelineRequestSchema,
  ExplicitPipelineRequestSchema
);
var MeExecutionRequestSchema = Schema3.Struct({
  contentType: Schema3.String,
  briefing: Schema3.Union(Schema3.String, Schema3.Record({ key: Schema3.String, value: Schema3.Unknown })),
  importedContext: Schema3.optional(Schema3.String),
  context: Schema3.optional(Schema3.Record({ key: Schema3.String, value: Schema3.Unknown })),
...
var SyncRunResponseSchema = Schema3.Struct({
  mode: ExecutionModeSchema,
  adapter: Schema3.String,
  model: Schema3.String,
  content: Schema3.String,
  contentType: Schema3.String,
  pipelineName: Schema3.String,
  qualityMode: QualityModeSchema,
  controls: Schema3.optional(ExecutionControlsSchema),
  telemetry: Schema3.optional(ExecutionTelemetrySchema),
  voice: Schema3.optional(ExecutionVoiceMetadataViewSchema),
  trace: Schema3.optional(Schema3.Unknown),
  idempotencyKey: Schema3.optional(Schema3.String)
});
var RunResponseSchema = Schema3.Union(SyncRunResponseSchema, AsyncRunResponseSchema);
var SyncExecutionViewSchema = Schema3.Struct({
  mode: Schema3.Literal("sync"),
  contentType: Schema3.String,
  pipelineName: Schema3.String,
  content: Schema3.String,
  adapter: Schema3.String,
...
var QueuedExecutionViewSchema = Schema3.Struct({
  jobId: Schema3.String,
  status: Schema3.Literal("queued"),
  contentType: Schema3.String,
  estimatedSteps: Schema3.Number,
  createdAt: Schema3.String,
  voice: ExecutionVoiceMetadataViewSchema
});
var ExecutionStatusViewSchema = Schema3.Struct({
  jobId: Schema3.String,
  status: JobStatusSchema,
  contentType: Schema3.String,
  progress: Schema3.NullOr(JobProgressSchema),
...
var ExecutionsPageViewSchema = Schema3.Struct({
  items: Schema3.Array(ExecutionStatusViewSchema),
  total: Schema3.Number,
  limit: Schema3.Number,
  offset: Schema3.Number
});
var ExecutionTransitionStartedSchema = Schema3.Struct({
  type: Schema3.Literal("started"),
  executionId: Schema3.String,
  snapshot: Schema3.optional(ExecutionStatusViewSchema),
  progress: JobProgressSchema,
  occurredAt: Schema3.String
});
var ExecutionTransitionProgressedSchema = Schema3.Struct({
  type: Schema3.Literal("progressed"),
  executionId: Schema3.String,
  snapshot: Schema3.optional(ExecutionStatusViewSchema),
  progress: JobProgressSchema,
  occurredAt: Schema3.String
});
var ExecutionTransitionCompletedSchema = Schema3.Struct({
  type: Schema3.Literal("completed"),
  executionId: Schema3.String,
  snapshot: Schema3.optional(ExecutionStatusViewSchema),
  result: JobResultSchema,
  occurredAt: Schema3.String
});
var ExecutionTransitionFailedSchema = Schema3.Struct({
  type: Schema3.Literal("failed"),
  executionId: Schema3.String,
  snapshot: Schema3.optional(ExecutionStatusViewSchema),
  error: JobErrorSchema,
  occurredAt: Schema3.String
});
var ExecutionTransitionSchema = Schema3.Union(
...
var ObservationFailureReasonSchema = Schema3.Literal(
  "reconnect_exhausted",
  "poll_fallback_exhausted",
  "timeout"
);
var ObservationFailureSchema = Schema3.Struct({
...
// ../../packages/contracts/src/content-types.ts
var ContentTypeDefinitionSchema = Schema4.Struct({
  id: Schema4.String,
  label: Schema4.String,
  steps: Schema4.Array(Schema4.String),
  defaultLanguage: Schema4.String,
  inputSchema: Schema4.Record({ key: Schema4.String, value: Schema4.Unknown })
});
var LanguageProfileSummarySchema = Schema4.Struct({
  code: Schema4.String,
  name: Schema4.String,
  defaults: Schema4.NullOr(
    Schema4.Struct({
      tone: Schema4.optional(Schema4.String),
      constraints: Schema4.optional(Schema4.Array(Schema4.String))
    })
  )
});
var ContentTypeFieldTypeSchema = Schema4.Literal("string", "text", "number", "boolean", "enum", "object", "array");
var ContentTypeFieldViewSchema = Schema4.Struct({
  key: Schema4.String,
  label: Schema4.String,
  type: ContentTypeFieldTypeSchema,
  required: Schema4.Boolean,
  highImpact: Schema4.Boolean,
  helpText: Schema4.optional(Schema4.String),
  options: Schema4.optional(Schema4.Array(Schema4.String))
});
var BriefingGuidanceViewSchema = Schema4.Struct({
  objective: Schema4.String,
  tips: Schema4.Array(Schema4.String),
  exampleBriefing: Schema4.String,
  commonMistakes: Schema4.Array(Schema4.String)
});
var ContentTypeCatalogItemViewSchema = Schema4.Struct({
  id: Schema4.String,
  label: Schema4.String,
  available: Schema4.Boolean,
  reasonCode: Schema4.optional(ReasonCodeSchema),
  defaultLanguage: Schema4.String,
  supportedLanguages: Schema4.Array(Schema4.String),
  steps: Schema4.Array(Schema4.String),
  inputSchema: Schema4.Array(ContentTypeFieldViewSchema),
  briefingGuidance: BriefingGuidanceViewSchema,
  briefingGuidanceByLanguage: Schema4.optional(
    Schema4.Record({ key: Schema4.String, value: BriefingGuidanceViewSchema })
  )
});
var ContentTypeCatalogCommercialSchema = Schema4.Struct({
  planTier: Schema4.String,
  allowedQualityModes: Schema4.Array(QualityModeSchema)
});
var ContentTypeCatalogViewSchema = Schema4.Struct({
  items: Schema4.Array(ContentTypeCatalogItemViewSchema),
  commercial: Schema4.optional(ContentTypeCatalogCommercialSchema)
});
...
var PreviewBriefingSchema = Schema5.Union(
  Schema5.String,
  Schema5.Record({ key: Schema5.String, value: Schema5.Unknown })
);
var GenerationPreviewRequestSchema = Schema5.Struct({
  contentType: Schema5.optional(Schema5.String),
  briefing: Schema5.optional(PreviewBriefingSchema),
  ...
  includeRecommendation: Schema5.optional(Schema5.Boolean)
});
var GenerationPreviewContentTypeOptionSchema = Schema5.Struct({
  id: Schema5.String,
  label: Schema5.String,
  allowed: Schema5.Boolean,
  blockedReason: Schema5.optional(Schema5.String)
});
var GenerationPreviewQualityModeOptionSchema = Schema5.Struct({
  id: QualityModeSchema,
  allowed: Schema5.Boolean,
  blockedReason: Schema5.optional(Schema5.String),
  creditPrice: Schema5.Number,
  recommended: Schema5.optional(Schema5.Boolean),
  recommendation: Schema5.optional(
    Schema5.Struct({
      reasonCodes: Schema5.Array(Schema5.String),
      explanation: Schema5.String
    })
  )
});
var GenerationPreviewRecommendationSchema = Schema5.Struct({
  qualityMode: QualityModeSchema,
  reasonCodes: Schema5.Array(Schema5.String),
  explanation: Schema5.String
});
var GenerationPricingSnapshotSchema = Schema5.Struct({
  quoteId: Schema5.String,
  policyVersion: Schema5.String,
  contentType: Schema5.String,
  qualityMode: QualityModeSchema,
  creditPrice: Schema5.Number
});
var GenerationPreviewResponseSchema = Schema5.Struct({
  pricingSnapshot: GenerationPricingSnapshotSchema,
  currentBalance: Schema5.Number,
  projectedBalanceAfterGeneration: Schema5.Number,
  recommendation: Schema5.optional(GenerationPreviewRecommendationSchema),
  options: Schema5.Struct({
    contentTypes: Schema5.Array(GenerationPreviewContentTypeOptionSchema),
    qualityModes: Schema5.Array(GenerationPreviewQualityModeOptionSchema)
  })
});
...
var BillingLedgerEntryTypeSchema = Schema6.Literal(
  "grant_cycle",
  "grant_rollover",
  "grant_topup",
  "reserve",
...
var BillingReferenceTypeSchema = Schema6.Literal(
  "subscription_cycle",
  "generation_cycle",
  "topup",
  "manual_adjustment",
...
var BillingRoundingSchema = Schema6.Literal("ceil_1_decimal");
var BillingModeCreditPolicySchema = Schema6.Struct({
  fast: Schema6.Number,
  balanced: Schema6.Number,
  strict: Schema6.Number
});
var BillingCreditPolicySchema = Schema6.Struct({
  baseCredits: BillingModeCreditPolicySchema,
  retrySurcharge: BillingModeCreditPolicySchema,
  rounding: BillingRoundingSchema,
  rolloverPercent: Schema6.Number,
  rolloverCap: Schema6.Number
});
var BillingLedgerEntrySchema = Schema6.Struct({
  subscriptionId: Schema6.String,
  accountId: Schema6.String,
  entryType: BillingLedgerEntryTypeSchema,
  creditsDelta: Schema6.Number,
  balanceAfter: Schema6.Number,
  referenceType: BillingReferenceTypeSchema,
  referenceId: Schema6.String,
  idempotencyKey: Schema6.String,
  metadata: Schema6.Record({ key: Schema6.String, value: Schema6.Unknown }),
  createdAt: Schema6.String
});
var BillingGenerationReservationStatusSchema = Schema6.Literal("reserved", "captured", "released");
var BillingGenerationReservationSchema = Schema6.Struct({
  reservationId: Schema6.String,
  generationCycleId: Schema6.String,
  subscriptionId: Schema6.String,
  accountId: Schema6.String,
  qualityMode: QualityModeSchema,
  retryCount: Schema6.Number,
  reservedCredits: Schema6.Number,
  status: BillingGenerationReservationStatusSchema,
  idempotencyKey: Schema6.String,
  metadata: Schema6.Record({ key: Schema6.String, value: Schema6.Unknown }),
  createdAt: Schema6.String,
  updatedAt: Schema6.String
});
var BillingWalletSchema = Schema6.Struct({
  accountId: Schema6.String,
  subscriptionId: Schema6.String,
  activeCycleId: Schema6.NullOr(Schema6.String),
  availableCredits: Schema6.Number,
  reservedCredits: Schema6.Number,
  pendingCredits: Schema6.Number,
  lifetimeGrantedCredits: Schema6.Number,
  lifetimeDebitedCredits: Schema6.Number
});
var BillingTopUpPackageSchema = Schema6.Struct({
  id: Schema6.String,
  credits: Schema6.Number,
  priceCents: Schema6.Number,
  currency: Schema6.String,
  description: Schema6.optional(Schema6.String)
});
var BillingCycleStateSchema = Schema6.Struct({
  cycleId: Schema6.String,
  subscriptionId: Schema6.String,
  accountId: Schema6.String,
  openedAt: Schema6.String,
  closedAt: Schema6.NullOr(Schema6.String),
  rolloverCredits: Schema6.Number,
  grantedCredits: Schema6.Number,
  expiredCredits: Schema6.Number
});
...
// src/product/ai-policy/ai-policy-schema.ts
var StepExecutionTypeSchema = Schema7.Literal("local", "llm");
var AIPolicyLifecycleSchema = Schema7.Literal("active", "legacy-supported");
var BillingPlanTierSchema = Schema7.Literal("free", "starter", "pro", "enterprise");
var RoutingFallbackConditionSchema = Schema7.Literal(
  "transport_error",
  "timeout",
  "invalid_request",
  "invalid_response",
...
var AIPolicyStepDocumentSchema = Schema7.Struct({
  name: Schema7.String,
  skill: Schema7.String,
  execution: StepExecutionTypeSchema,
  routingProfile: Schema7.optional(Schema7.String),
  override: Schema7.optional(
    Schema7.Struct({
      from: Schema7.Literal("local"),
      to: Schema7.Literal("llm"),
      reason: Schema7.String
    })
  )
});
var AIPolicyCatalogDocumentSchema = Schema7.Struct({
  policyVersion: Schema7.String,
  routingProfiles: Schema7.Array(
    Schema7.Struct({
      id: Schema7.String,
      preferredAttempts: Schema7.NonEmptyArray(
        Schema7.Struct({
          provider: Schema7.String,
          model: Schema7.String,
          timeoutMs: Schema7.optional(Schema7.Number)
        })
      ),
      fallbackAttempts: Schema7.Array(
        Schema7.Struct({
          provider: Schema7.String,
          model: Schema7.String,
          timeoutMs: Schema7.optional(Schema7.Number)
      ...
      operationalConstraints: Schema7.Struct({
        fallbackOn: Schema7.NonEmptyArray(RoutingFallbackConditionSchema)
      })
    })
  ),
  contentTypes: Schema7.Array(
    Schema7.Struct({
      id: Schema7.String,
      label: Schema7.String,
      defaultLanguage: Schema7.String,
      pipelineType: PipelineTypeSchema
    })
  ),
  pipelines: Schema7.Array(
    Schema7.Struct({
      pipelineType: PipelineTypeSchema,
      contentType: Schema7.String,
      defaultLanguage: Schema7.String,
      defaultQualityMode: QualityModeSchema,
      steps: Schema7.Array(AIPolicyStepDocumentSchema)
    })
  )
});
var AIPolicyPricingDocumentSchema = Schema7.Struct({
  policyVersion: Schema7.String,
  lifecycle: AIPolicyLifecycleSchema,
  pricing: Schema7.Array(
    Schema7.Struct({
      planTier: BillingPlanTierSchema,
      contentType: Schema7.String,
      qualityMode: QualityModeSchema,
      creditPrice: Schema7.Number
    })
  )
});
var AIPolicyManifestSchema = Schema7.Struct({
  activeVersion: Schema7.String,
  versions: Schema7.Array(
    Schema7.Struct({
      version: Schema7.String,
      lifecycle: AIPolicyLifecycleSchema,
      catalogPath: Schema7.String,
      pricingPath: Schema7.String
    })
  )
});
...
function loadJsonFile(path, schema) {
  return Effect92.gen(function* () {
    ...
    const decodedJson = yield* Effect92.try({
      try: () => JSON.parse(raw),
    ...
    return yield* Schema8.decodeUnknown(schema)(decodedJson).pipe(
      Effect92.mapError(
...
import { Effect as Effect93 } from "effect";
...
function persistMemoryValue(database, key, value, at) {
  return database.memories.put(
    ...
    Effect93.catchAll(swallowWithDiagnostic({
...
function createActivePolicyPointerController(args) {
  return Effect94.gen(function* () {
    ...
    const reloadPointer = () => Effect94.gen(function* () {
    ...
    const ensureFreshPointer = () => Effect94.gen(function* () {
    ...
    return {
      ...
      activatePolicyVersion: (activation) => args.database.transaction(
        (trxDatabase) => Effect94.gen(function* () {
...
function resolvePolicyVersion(index, policyVersion, context) {
  ...
  if (policy) {
    return Effect95.succeed(policy);
  }
  return Effect95.fail(
...
function resolvePolicyPipeline(policy, contentType) {
  ...
  return Effect95.succeed(policy.catalog[contentTypeDefinition.pipelineType]);
}
...
function validateExplicitPipelineDefinition(policyPipeline, pipeline, policyVersion) {
  return Effect96.gen(function* () {
    if (pipeline.steps.length !== policyPipeline.steps.length) {
      return yield* Effect96.fail(
        new BackendAIPolicyCatalogError({
          policyVersion,
          pipelineName: pipeline.name,
...
function createBackendAIPolicyService(options) {
  ...
  return Effect99.gen(function* () {
    ...
    const resolveSelectedPolicyVersion = () => Effect99.gen(function* () {
    ...
    const resolveActivePolicy = () => Effect99.gen(function* () {
    ...
    return {
      ...
      activatePolicyVersion: (args) => Effect99.gen(function* () {
      ...
      recommendFuturePolicyVersion: () => Effect99.gen(function* () {
        const selectedPolicyVersion = yield* resolveSelectedPolicyVersion();
      ...
      validatePipelineRequest: (request) => Effect99.gen(function* () {
      ...
      resolvePricingEnvelope: (args) => Effect99.gen(function* () {
      ...
      resolveExecutionSnapshot: (args) => Effect99.gen(function* () {
        const selectedPolicyVersion = args.attachedPolicyVersion ?? options.attachedPolicyVersion ?? (yield* resolveSelectedPolicyVersion());
...
function loadBackendAIPolicyService(config, options = {}, dependencies = {}) {
  return Effect100.gen(function* () {
    ...
    if (!dependencies.database) {
      return yield* Effect100.die("Backend AI policy service requires a database client");
    }
...
import { Effect as Effect101, Schema as Schema11 } from "effect";
...
var SafetyPolicyLifecycleSchema = Schema9.Literal("active", "legacy-supported");
var SafetyPolicyFamilySchema = Schema9.Literal(
  "input",
  "imported_context",
  "step_scope",
  "consent",
...
var OperationalOverrideTargetFamilySchema = Schema9.Literal(
  "input",
  "imported_context",
  "step_scope",
  "consent",
...
var SafetyDecisionOutcomeSchema = Schema9.Literal(
  "approve",
  "sanitize",
  "quarantine",
  "block",
...
var SafetyOverrideabilitySchema = Schema9.Literal("never", "one_shot", "time_limited");
var SafetyEvidenceBoundarySchema = Schema9.Literal("input", "scope", "output", "consent", "override");
var OperationalOverrideTargetBoundarySchema = Schema9.Literal("input", "scope", "output", "consent");
var SafetyClassificationCategorySchema = Schema9.Literal(
...
var OperationalOverrideLifecycleModeSchema = Schema9.Literal("one_shot", "time_limited");

// src/product/safety-policy/safety-policy-schema.ts
var SafetyPolicyFamilyDocumentSchema = Schema10.Struct({
  family: SafetyPolicyFamilySchema,
  defaultOutcome: SafetyDecisionOutcomeSchema,
  allowedOutcomes: Schema10.NonEmptyArray(SafetyDecisionOutcomeSchema),
  overrideability: SafetyOverrideabilitySchema,
  evidenceBoundary: SafetyEvidenceBoundarySchema,
  detectorAdapters: Schema10.Array(Schema10.String),
  nonOverridableCategories: Schema10.optional(Schema10.Array(SafetyClassificationCategorySchema)),
  maxOverrideWindowMinutes: Schema10.optional(Schema10.Number.pipe(Schema10.positive()))
});
var SafetyClassificationDocumentSchema = Schema10.Struct({
  category: SafetyClassificationCategorySchema,
  defaultOutcome: SafetyDecisionOutcomeSchema,
  minimizationRequired: Schema10.Boolean,
  description: Schema10.String
});
var SafetyDetectorAdapterDocumentSchema = Schema10.Struct({
  id: Schema10.String,
  kind: Schema10.Literal("classifier", "scanner", "heuristic"),
  optional: Schema10.Boolean,
  boundaries: Schema10.NonEmptyArray(SafetyEvidenceBoundarySchema)
});
var SafetyPolicyDocumentSchema = Schema10.Struct({
  policyVersion: Schema10.String,
  lifecycle: SafetyPolicyLifecycleSchema,
  unknownInputOutcome: Schema10.Literal("block"),
  evidenceBoundaries: Schema10.NonEmptyArray(SafetyEvidenceBoundarySchema),
  families: Schema10.Array(SafetyPolicyFamilyDocumentSchema),
  classifications: Schema10.Array(SafetyClassificationDocumentSchema),
  detectorAdapters: Schema10.Array(SafetyDetectorAdapterDocumentSchema)
});
var SafetyPolicyManifestSchema = Schema10.Struct({
  activeVersion: Schema10.String,
  versions: Schema10.Array(
    Schema10.Struct({
      version: Schema10.String,
      lifecycle: SafetyPolicyLifecycleSchema,
      policyPath: Schema10.String
    })
  )
});
...
function loadResolvedSafetyPolicyDocuments(manifestPath) {
  return Effect101.gen(function* () {
    ...
    const versions = yield* Effect101.all(
...
function loadJsonFile2(path, schema) {
  return Effect101.gen(function* () {
    ...
    const decodedJson = yield* Effect101.try({
      try: () => JSON.parse(raw),
    ...
    return yield* Schema11.decodeUnknown(schema)(decodedJson).pipe(
      Effect101.mapError(
...
import { Effect as Effect102 } from "effect";
...
function resolveSafetyPolicyFamily(policy, family) {
  ...
  if (definition) {
    return Effect102.succeed(definition);
  }
  return Effect102.fail(
    new BackendSafetyPolicyDefinitionError({
      policyVersion: policy.version,
      family,
...
function loadBackendSafetyPolicyService(config, options = {}) {
  return Effect103.gen(function* () {
    ...
    return {
      getActivePolicy: () => Effect103.succeed(index.activePolicy),
      listPolicyVersions: () => index.versionSummaries,
...
function createBackendVoiceFieldProtectionService(options) {
  ...
  return {
    protectVoiceExample: (record) => Effect104.gen(function* () {
    ...
    unprotectVoiceExample: (record) => Effect104.gen(function* () {
    ...
    protectVoiceExampleBatch: (record) => Effect104.gen(function* () {
      const items = yield* Effect104.forEach(record.items, (item) => {
        if (!item.stagedInput) {
          return Effect104.succeed(item);
        }
    ...
    unprotectVoiceExampleBatch: (record) => Effect104.gen(function* () {
      const items = yield* Effect104.forEach(record.items, (item) => {
        if (!item.stagedInput) {
          return Effect104.succeed(item);
        }
...
function protectOptionalStringField(userId, fieldName, value, key) {
  if (value === void 0) {
    return Effect104.succeed(void 0);
  }
...
function unprotectStringFieldWithFallback(userId, fieldName, value, key, previousKey) {
  if (!isProtectedVoiceField(value)) {
    return Effect104.succeed(value);
  }
  return decryptProtectedString(userId, fieldName, value, key).pipe(
    Effect104.catchAll((currentError) => {
      if (!previousKey) {
        return Effect104.fail(currentError);
      }
...
function createProtectedVoiceTrainingDatabaseClient(database, protection) {
  const baseClient = {
    ...database,
    voiceExamples: {
      create: (record, version) => Effect105.gen(function* () {
      ...
      save: (record) => Effect105.gen(function* () {
        const protectedRecord = yield* protection.protectVoiceExample(record);
        const stored = yield* database.voiceExamples.save(protectedRecord);
        return yield* protection.unprotectVoiceExample(stored);
      }),
      get: (id) => Effect105.flatMap(
        database.voiceExamples.get(id),
        (record) => record ? protection.unprotectVoiceExample(record) : Effect105.succeed(void 0)
      ),
      listByUser: (userId) => Effect105.flatMap(
        database.voiceExamples.listByUser(userId),
        (records) => Effect105.forEach(records, (record) => protection.unprotectVoiceExample(record), { concurrency: 1 })
      ),
    ...
    voiceExampleBatches: {
      ...
      get: (id) => Effect105.flatMap(
        ...
        (record) => record ? protection.unprotectVoiceExampleBatch(record) : Effect105.succeed(void 0)
      ),
      listByUser: (userId) => Effect105.flatMap(
        database.voiceExampleBatches.listByUser(userId),
        (records) => Effect105.forEach(records, (record) => protection.unprotectVoiceExampleBatch(record), { concurrency: 1 })
      ),
...
import { Effect as Effect112 } from "effect";
...
import { Layer as Layer11, Effect as Effect111 } from "effect";
...
// src/product/voice/voice-rebuild-service.ts
function createBackendVoiceRebuildService(database, now, observability, logger, voiceConsent) {
  ...
  const startRunner = (userId, state) => {
    ...
    state.activePromise = (async () => {
      try {
        while (state.queued) {
          state.queued = false;
          await Effect112.runPromise(processUserRebuild(database, userId, now, observability, logger, voiceConsent));
          if (state.queued) {
            await Effect112.runPromise(markRebuildQueued(database, userId, now));
          }
        }
      } finally {
        state.running = false;
  ...
  return {
    schedule(userId) {
      return Effect112.sync(() => {
        ...
        void Effect112.runPromise(markRebuildQueued(database, userId, now)).catch(() => void 0);
        startRunner(userId, state);
      });
    },
    drain(userId) {
      return Effect112.promise(() => waitForIdle(userId));
    }
  };
}
function processUserRebuild(database, userId, now, observability, logger, voiceConsent) {
  return Effect112.gen(function* () {
    if (voiceConsent) {
      const consentStatus = yield* voiceConsent.getConsentStatus(userId).pipe(
        Effect112.orElseSucceed(() => ({ granted: false, revokedAt: void 0 }))
      );
    ...
    yield* database.voiceProfiles.put(derivedState.profile).pipe(Effect112.orDie);
    yield* database.voiceProfileDiagnostics.put(derivedState.diagnostics).pipe(Effect112.orDie);
...
function markRebuildQueued(database, userId, now) {
  return Effect112.gen(function* () {
    ...
    yield* database.voiceProfileDiagnostics.put(diagnostics).pipe(Effect112.orDie);
  });
}
...
import { Effect as Effect115 } from "effect";
...
function validateVoiceExampleInput(input) {
  ...
  return Effect113.succeed(void 0);
}
...
function toVoiceProfileScreenView(profile, diagnostics) {
  return {
...
import { Effect as Effect114 } from "effect";
...
function requireOpenBatch(database, userId, batchId, now) {
  return Effect114.gen(function* () {
    ...
    if (!batch || batch.userId !== userId) {
      return yield* Effect114.fail(new VoiceBatchNotFoundError({ batchId }));
    }
    if (batch.status !== "open" || batch.expiresAt <= now().toISOString()) {
      return yield* Effect114.fail(
...
function commitAcceptedBatchItems(database, userId, batch, targetProfileVersion, now) {
  return Effect114.gen(function* () {
    ...
    for (const item of batch.items) {
      ...
      const stored = yield* database.voiceExamples.create(example).pipe(Effect114.orDie);
...
function commitExpiredBatch(database, batch, now, observability, logger, voiceConsent) {
  return Effect114.gen(function* () {
    ...
    const { createdExamples, saved } = yield* database.transaction(
      (trxDatabase) => Effect114.gen(function* () {
      ...
      Effect114.catchTag("DatabaseTransactionInvariantError", (error) => Effect114.die(error))
    );
...
function listAllVoiceBatches(database) {
  return Effect114.sync(() => Object.values(database.snapshot().voiceExampleBatches));
}
  ...
  return {
    createBatch(userId, options = {}) {
      return Effect115.gen(function* () {
        ...
        const created = yield* database.voiceExampleBatches.create(batch).pipe(Effect115.orDie);
    ...
    commitBatch(userId, batchId) {
      return Effect115.gen(function* () {
        ...
        const { createdExamples, saved } = yield* database.transaction(
          (trxDatabase) => Effect115.gen(function* () {
          ...
          Effect115.catchTag("DatabaseTransactionInvariantError", (error) => Effect115.die(error))
        );
    ...
    autoCommitExpiredBatches(userId) {
      return Effect115.gen(function* () {
        ...
        const committed = yield* Effect115.forEach(
          expired,
        ...
        yield* Effect115.forEach(affectedUsers, (affectedUserId) => voiceRebuild.schedule(affectedUserId), {
          concurrency: 1,
          discard: true
        });
        return committed;
    ...
    createExample(userId, input) {
      return Effect117.gen(function* () {
        ...
        const created = yield* database.transaction(
          (trxDatabase) => Effect117.gen(function* () {
            ...
            const stored = yield* trxDatabase.voiceExamples.create(example).pipe(Effect117.orDie);
            yield* persistBackendAuditEvent(trxDatabase, {
          ...
          Effect117.catchTag("DatabaseTransactionInvariantError", (error) => Effect117.die(error))
        );
  ...
  switch (contentType) {
    case "linkedin-post":
      return {
        ...
        lexicon: ["observa\xE7\xE3o", "experi\xEAncia", "contexto"]
      };
    case "twitter-thread":
      return {
        tone: "concise",
...
function buildVoiceHints(profile, matchingExamples, pinnedMatchingExamples, context, confidence, adaptationMode, domainProfile) {
  ...
  const antiPatternsExplicit = explicitFromExamples.length > 0 ? explicitFromExamples : [...preset.antiPatterns];
...
function pickSignals(profileSignals, presetSignals, limit) {
  ...
  const merged = unique4([...preferred, ...profileSignals, ...presetSignals]);
...
// src/product/voice/voice-effective-resolution.ts
function resolveEffectiveVoice(database, userId, context, now, observability, logger, voiceConsent) {
  return Effect118.gen(function* () {
    if (voiceConsent) {
      const consentStatus = yield* voiceConsent.getConsentStatus(userId).pipe(
        Effect118.orElseSucceed(() => ({ granted: false, revokedAt: void 0 }))
      );
      if (!consentStatus.granted || consentStatus.revokedAt !== void 0) {
        return void 0;
      }
...
// src/product/voice/voice-service.ts
function createBackendVoiceService(database, voiceRebuild, now, observability, logger, voiceConsent) {
  ...
  return {
    getProfileScreen(userId) {
      return Effect119.gen(function* () {
        if (voiceConsent) {
          const consentStatus = yield* voiceConsent.getConsentStatus(userId).pipe(
            Effect119.orElseSucceed(() => ({ granted: false, revokedAt: void 0 }))
          );
          if (!consentStatus.granted || consentStatus.revokedAt !== void 0) {
            return void 0;
          }
    ...
    resolveEffectiveVoice(userId, context) {
      return resolveEffectiveVoice(database, userId, context, now, observability, logger, voiceConsent);
    },
    ...lifecycle,
    ...batches
  };
...
import { Effect as Effect120, Ref as Ref3 } from "effect";
var emptyCounters = {
...
function createBackendApplicationUserMemoryRepository() {
  ...
  return {
    ...
    findById(id) {
      return Effect121.sync(() => store.get(id));
    }
  };
}
...
function createBackendOperatorMemoryRepository() {
  ...
  return {
    findById(id) {
      return Effect122.sync(() => store.get(id));
    },
    create(args) {
      return Effect122.sync(() => {
        const operator = {
          id: args.id,
  ...
  return {
    findById(id) {
      return Effect124.gen(function* () {
        const row = yield* Effect124.tryPromise({
          try: () => db.selectFrom("operators").where("id", "=", id).selectAll().executeTakeFirst(),
          catch: () => void 0
        }).pipe(Effect124.catchAll(() => Effect124.succeed(void 0)));
...
function createRedisTrafficLimitStore(redis) {
  return {
    increment(key, ttlMs) {
      ...
      }).pipe(Effect125.catchAll(() => Effect125.succeed(1)));
    }
  };
}
...
async function main() {
  ...
  const services = await Effect127.runPromise(createBackendProductServices(config));
  await Effect127.runPromise(registerBackendBillingPlans(services.billing));
  const entitlement = await Effect127.runPromise(
