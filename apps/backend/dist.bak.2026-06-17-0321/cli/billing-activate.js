...
import { Effect as Effect113, Schema as Schema13 } from "effect";
var ReasoningExtractionError = class extends Error {
  _tag = "ReasoningExtractionError";
  constructor(message) {
    super(message);
...
function groupExamplesByContentType(examples) {
  ...
  for (const example of examples.filter((item) => item.state === "active")) {
    const contentTypes = example.explicitContentType ? [example.explicitContentType] : example.effectiveContentTypeHints.length > 0 ? [...example.effectiveContentTypeHints] : ["general"];
...
function extractReasoningSignature(args) {
  return Effect113.gen(function* () {
    ...
    for (const attempt of args.attempts) {
      const completion = yield* Effect113.either(
      ...
      const parsed = yield* parseExtractionResponse(completion.right.response.text).pipe(Effect113.either);
      if (parsed._tag === "Right") {
        return filterFormatExpressionsByCoverage(parsed.right, grouped);
      }
      lastError = parsed.left;
    }
    return yield* Effect113.fail(lastError ?? new ReasoningExtractionError("Reasoning extraction failed"));
  });
}
function parseExtractionResponse(content) {
  return Effect113.gen(function* () {
    ...
    Effect113.catchAll(
      (error) => Effect113.fail(
        error instanceof ReasoningExtractionError ? error : new ReasoningExtractionError(
...
function buildExtractionPrompt(grouped, examples) {
  const sections = Object.entries(grouped).map(([contentType, groupedExamples]) => {
    const exampleBlocks = groupedExamples.map((example, index) => `Example ${index + 1}:
...
function resolveReasoningLanguageInstruction(examples) {
...
var REASONING_EXTRACTION_SYSTEM_PROMPT = [
  ...
  const startRunner = (userId, state) => {
    ...
    state.activePromise = (async () => {
      try {
        while (state.queued) {
          state.queued = false;
          await Effect114.runPromise(
            processUserRebuild(database, userId, now, observability, logger, voiceConsent, dependencies)
          );
          if (state.queued) {
            await Effect114.runPromise(markRebuildQueued(database, userId, now));
          }
        }
      } finally {
        state.running = false;
  ...
  return {
    schedule(userId) {
      return Effect114.sync(() => {
        ...
        void Effect114.runPromise(markRebuildQueued(database, userId, now)).catch(() => void 0);
        startRunner(userId, state);
      });
    },
    drain(userId) {
      return Effect114.promise(() => waitForIdle(userId));
    }
  };
}
  ...
  return Effect114.gen(function* () {
    if (voiceConsent) {
      const consentStatus = yield* voiceConsent.getConsentStatus(userId).pipe(
        Effect114.orElseSucceed(() => ({ granted: false, revokedAt: void 0 }))
      );
    ...
    yield* database.voiceProfiles.put(derivedState.profile).pipe(Effect114.orDie);
    yield* database.voiceProfileDiagnostics.put(derivedState.diagnostics).pipe(Effect114.orDie);
...
function markRebuildQueued(database, userId, now) {
  return Effect114.gen(function* () {
    ...
    yield* database.voiceProfileDiagnostics.put(diagnostics).pipe(Effect114.orDie);
  });
}
...
function createBackendProviderTransport(config) {
  ...
  return {
    complete: (providerRequest) => Effect115.gen(function* () {
      ...
      const headers = yield* Effect115.try({
      ...
      const response = yield* Effect115.tryPromise({
        try: (signal) => {
      ...
      return yield* Effect115.tryPromise({
        try: () => response.json(),
        catch: (cause) => new AIAdapterTransportError({
          provider: providerRequest.provider,
...
function createTestProviderTransport() {
  return {
    complete: (providerRequest) => Effect115.succeed({
      choices: [
        {
          message: {
...
function readResponseBody(response) {
  return Effect115.tryPromise({
    try: () => response.text(),
    catch: () => ""
  }).pipe(
    Effect115.map((value) => typeof value === "string" ? value : ""),
    Effect115.orElseSucceed(() => "")
  );
}
...
function validateVoiceExampleInput(input) {
  ...
  return Effect116.succeed(void 0);
}
...
function toVoiceProfileScreenView(profile, diagnostics, options) {
  return {
    ...
    ...options?.includeReasoning && profile.coreReasoningSignature ? { reasoning: toVoiceReasoningPresentationView(profile) } : {}
  };
}
function toVoiceReasoningPresentationView(profile) {
...
function requireOpenBatch(database, userId, batchId, now) {
  return Effect117.gen(function* () {
    ...
    if (!batch || batch.userId !== userId) {
      return yield* Effect117.fail(new VoiceBatchNotFoundError({ batchId }));
    }
...
function commitAcceptedBatchItems(database, userId, batch, targetProfileVersion, now) {
  return Effect117.gen(function* () {
    ...
    for (const item of batch.items) {
      ...
      const stored = yield* database.voiceExamples.create(example).pipe(Effect117.orDie);
...
function listAllVoiceBatches(database) {
  return Effect117.sync(() => Object.values(database.snapshot().voiceExampleBatches));
}
  ...
  return {
    createBatch(userId, options = {}) {
      return Effect118.gen(function* () {
        ...
        const created = yield* database.voiceExampleBatches.create(batch).pipe(Effect118.orDie);
    ...
    commitBatch(userId, batchId) {
      return Effect118.gen(function* () {
        ...
        const { createdExamples, saved } = yield* database.transaction(
          (trxDatabase) => Effect118.gen(function* () {
          ...
          Effect118.catchTag("DatabaseTransactionInvariantError", (error) => Effect118.die(error))
        );
    ...
    autoCommitExpiredBatches(userId) {
      return Effect118.gen(function* () {
        ...
        const committed = yield* Effect118.forEach(
          expired,
        ...
        yield* Effect118.forEach(affectedUsers, (affectedUserId) => voiceRebuild.schedule(affectedUserId), {
          concurrency: 1,
          discard: true
        });
        return committed;
      ...
      return Effect120.gen(function* () {
        ...
        const created = yield* database.transaction(
          (trxDatabase) => Effect120.gen(function* () {
            ...
            const stored = yield* trxDatabase.voiceExamples.create(example).pipe(Effect120.orDie);
            yield* persistBackendAuditEvent(trxDatabase, {
...
function resolveEffectiveVoice(database, userId, context, now, observability, logger, voiceConsent, options) {
  return Effect121.gen(function* () {
    if (voiceConsent) {
      const consentStatus = yield* voiceConsent.getConsentStatus(userId).pipe(
        Effect121.orElseSucceed(() => ({ granted: false, revokedAt: void 0 }))
      );
      if (!consentStatus.granted || consentStatus.revokedAt !== void 0) {
        return void 0;
      }
  ...
  return {
    getProfileScreen(userId) {
      return Effect122.gen(function* () {
        if (voiceConsent) {
          const consentStatus = yield* voiceConsent.getConsentStatus(userId).pipe(
            Effect122.orElseSucceed(() => ({ granted: false, revokedAt: void 0 }))
          );
          if (!consentStatus.granted || consentStatus.revokedAt !== void 0) {
            return void 0;
          }
...
import { Effect as Effect123, Ref as Ref3 } from "effect";
var emptyCounters = {
...
function createBackendApplicationUserMemoryRepository() {
  ...
  return {
    ...
    findById(id) {
      return Effect124.sync(() => store.get(id));
    }
  };
}
...
function createBackendOperatorMemoryRepository() {
  ...
  return {
    findById(id) {
      return Effect125.sync(() => store.get(id));
    },
    create(args) {
      return Effect125.sync(() => {
        const operator = {
          id: args.id,
  ...
  return {
    findById(id) {
      return Effect127.gen(function* () {
        const row = yield* Effect127.tryPromise({
          try: () => db.selectFrom("operators").where("id", "=", id).selectAll().executeTakeFirst(),
          catch: () => void 0
        }).pipe(Effect127.catchAll(() => Effect127.succeed(void 0)));
        return row ? parseOperator(row) : void 0;
      }).pipe(Effect127.orDie);
    },
    create(args) {
      return Effect127.gen(function* () {
        const now = (/* @__PURE__ */ new Date()).toISOString();
...
function createRedisTrafficLimitStore(redis) {
  return {
    increment(key, ttlMs) {
      ...
      }).pipe(Effect128.catchAll(() => Effect128.succeed(1)));
    }
  };
}
...
async function main() {
  ...
  const services = await Effect130.runPromise(createBackendProductServices(config));
  await Effect130.runPromise(registerBackendBillingPlans(services.billing));
  const entitlement = await Effect130.runPromise(
