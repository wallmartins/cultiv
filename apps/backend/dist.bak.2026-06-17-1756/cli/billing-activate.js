  ...
  return {
    getProfileScreen(userId) {
      return Effect125.gen(function* () {
        if (voiceConsent) {
          const consentStatus = yield* voiceConsent.getConsentStatus(userId).pipe(
            Effect125.orElseSucceed(() => ({ granted: false, revokedAt: void 0 }))
          );
          if (!consentStatus.granted || consentStatus.revokedAt !== void 0) {
            return void 0;
          }
...
import { Effect as Effect126, Ref as Ref3 } from "effect";
var emptyCounters = {
...
function createBackendApplicationUserMemoryRepository() {
  ...
  return {
    ...
    findById(id) {
      return Effect127.sync(() => store.get(id));
    }
  };
}
...
function createRedisTrafficLimitStore(redis) {
  return {
    increment(key, ttlMs) {
      ...
      }).pipe(Effect131.catchAll(() => Effect131.succeed(1)));
    }
  };
}
...
async function main() {
  ...
  const services = await Effect133.runPromise(createBackendProductServices(config));
  await Effect133.runPromise(registerBackendBillingPlans(services.billing));
  const entitlement = await Effect133.runPromise(
