...
function parseArgs(argv) {
  ...
  const outPath = argv.includes("--out") ? resolve3(argv[argv.indexOf("--out") + 1] ?? "calibration-sweep-manifest.json") : resolveCalibrationRepoPath("docs/superpowers/reports/calibration-sweep-manifest.json");
...
async function main() {
  ...
  for (const cell of profile.cells) {
    for (let runIndex = 0; runIndex < cell.repeats; runIndex += 1) {
      ...
      mkdirSync(dirname3(outPath), { recursive: true });
