...
import { Context, Effect as Effect5, Layer } from "effect";
...
function backfillBillingSnapshotIntoRelationalTables(db) {
  return Effect6.gen(function* () {
    const snapshotRow = yield* Effect6.tryPromise({
    ...
    const existingPlans = yield* Effect6.tryPromise({
...
async function up(db) {
  ...
  await Effect7.runPromise(backfillBillingSnapshotIntoRelationalTables(db));
}
