...
export function AppSegmentedControl({
  ...
  return (
    <div
      className={cn(
        "relative flex rounded-[var(--workspace-radius-md)] border border-border-subtle/70 bg-surface/70 p-1 backdrop-blur-sm",
        className
      )}
      role="radiogroup"
    >
      <span
        aria-hidden
        className="app-shell-nav-pill pointer-events-none absolute rounded-[calc(var(--workspace-radius-md)-0.2rem)] bg-moss/14 shadow-[inset_0_0_0_1px_color-mix(in_srgb,var(--color-moss)_28%,transparent)]"
        ...
        return (
          ...
          >
            ...
            <Text
              as="span"
              variant="meta"
              className={cn(selected ? "font-semibold text-foreground" : "text-muted-foreground")}
            >
              {option.label}
            </Text>
          </label>
