  ...
  it("pairs pt and en home routes with hreflang alternates", () => {
    ...
    expect(en.meta).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ name: "og:image", content: expect.stringMatching(/\/cultiv-og\.svg$/) }),
        ...
        expect.objectContaining({ name: "twitter:image", content: expect.stringMatching(/\/cultiv-og\.svg$/) })
      ])
    );
  });
