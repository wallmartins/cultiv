...
import { createInMemoryJobRepository } from "../../apps/backend/src/jobs/in-memory-job-repository.js";
...
describe("execution list filters", () => {
  it("normalizes query filters and matches jobs by period, status, and content type", () => {
  ...
  it("filters in-memory jobs before pagination and total", () => {
    ...
    const recent = Effect.runSync(
      repository.createQueuedJob(createPipelineRequest("user-a", "twitter-thread"), {
        createdAt: "2026-06-20T00:00:00.000Z"
      })
    );
    Effect.runSync(repository.completeJob(old.jobId, { output: "ok" }, "2026-05-01T01:00:00.000Z"));
    Effect.runSync(repository.failJob(recent.jobId, { message: "boom" }, "2026-06-20T01:00:00.000Z"));
  ...
  it("passes decoded filters from GET /me/executions to listJobsForUser", async () => {
    ...
    const response = await app.request(
      "/me/executions?period=30d&status=done&contentType=newsletter&limit=5&offset=10",
      {
